...
- **3T method** where GMOS gate and drain are not necessarily the same.
...
1. **Single ended 2T** experiments:
    - Signal generated pulse burst
    - Noise measurement using LIA
...
The purpose of this setup is to evaluate the temperature dependence of a single GMOS as measured using the LIA when applying constant current to the GMOS drain port. The available parameters which may effect the experiment are the **LIA's amplitude, frequency and offset**, as well as the **signal generators pulse width and amplitude**. For a selected list of parameters listed above. The setup connection for this experiment has an SCU connected to a resistor with constant size, other end of the resistor is connected to the NMOS connected in 2T. The Drain of the NMOS is connected to the A input of the LIA. the experiment will execute as follows:

1. Setup of the ATEs, including resetting the devices, enabling ESD protection (usually the PSU). Setting LIA reference to internal.
2. Setting the bias of the NMOS using the SCU
3. Setting the LIA parameters (frequency of reference, degree of low pass filter, sensitivity).
4. Measurement of X and Y parameters with respect to the frequency
5. Continue sweeping frequency from the start to the stop according to what is selected as the experiment argument (can be either linear or log).
6. Store data to the `Results` folder.

#### Noise Measurement using LIA

The purpose of this setup is to measure the PSD of the NMOS transistor that is used as the temperature measurement device in the GMOS system. The important parameters that this experiment aims to estimate is the power of the **white** and **flicker** noise attributes of the NMOS device. The experiment will be executed as follows:

1. Setup of the ATEs, including resetting the devices, enabling ESD protection (usually the PSU).
2. Setting the LIA parameters.
3. Preparation of the signal generated pulse (but not triggering yet).
4. Enabling the stream of X and Y data from the LIA to the computer and initializing a daemon to capture the output to a `.csv` file containing sample data with respect to the time.
...
| B2962A | start_monitor_daemon | channel (enum) | daemon handler |
| | | | |
